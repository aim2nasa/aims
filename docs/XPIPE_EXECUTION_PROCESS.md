...
### Sprint 0-1: 보안 이슈 해결 — ✅ 완료 (2026-03-19)

| 단계 | 결과 |
|------|------|
| 분석 | Alex — 4개 보안 항목 코드 분석 + 실행 설계 (예상 3시간 15분) |
| 실행 | 4건 수정 완료: SSE 인증 2건, chat-routes userId, personal-files DI 리팩토링, CORS 제한 |
| 검증 | Gini — PASS with Minor. 1,052 테스트 전원 PASS. Minor(CORS) 즉시 해소 |
| 갱신 | 신규 Major 발견: `customer-relationships-routes.js:591` JWT 미사용 → Sprint 0-2에 포함 |

**실측 데이터**: 예상보다 빠르게 완료. personal-files DI 리팩토링은 패턴이 동일하여 일괄 치환 가능했음.

### Sprint 0-2: 잔여 보안 + God Function 분해 (1~2주)

| 단계 | 내용 |
|------|------|
| 분석 | Alex — (1) `customer-relationships-routes.js:591` 인증 구조 분석 (2) doc_prep_main.py 1,777줄 전체 분석, 스테이지 경계 식별 |
| 실행 | (1) customer-relationships JWT 인증 추가 (2) 오케스트레이터 + 단계별 함수로 분해 |
| 검증 | Gini — 잔여 보안 해소 확인 + 분해 후 기존 동작 보존 확인 (E2E) |
