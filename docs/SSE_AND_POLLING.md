...
| 3 | 내 문서 | `PersonalFilesView.tsx` | SSE | 🟡 중간 | ✅ 완료 |
| 4 | 문서 등록 | `DocumentRegistrationView.tsx` | SSE | 🟡 중간 | ✅ 완료 |
| 5 | 문서 처리 현황 | `DocumentStatusProvider.tsx` | SSE | 🔴 높음 | ✅ 완료 |
...
#### 3. 내 문서 (PersonalFilesView) ✅
- **완료일**: 2025-12-19
- **변경**: 5초 폴링 → SSE 실시간
- **파일**:
  - `server.js`: SSE 엔드포인트 (`/api/personal-files/stream`), webhook 엔드포인트 (`/api/webhooks/personal-files-change`)
  - `personal-files-routes.js`: 파일/폴더 생성/삭제/이동/이름변경 시 webhook 호출
  - `usePersonalFilesSSE.ts`: SSE 훅
  - `PersonalFilesView.tsx`: 폴링 코드 제거, SSE 훅 사용
  - `personalFilesService.ts`: `moveDocument()`, `renameDocument()` 메서드 추가
- **특이사항**:
  - Personal Files routes가 별도 모듈이므로 webhook 패턴 사용
  - userId 기반 SSE 구독 (customerId 대신)
- **추가 수정** (버그 픽스):
  - 파일 목록 표시: `fileScope='onlyMyFiles'` 파라미터 추가 (기존 excludeMyFiles가 기본값이라 내 파일이 안보였음)
  - 문서 이동 API: `customerId` (ObjectId) vs `userId` (string) 비교 수정 (`$expr: { $eq: [{ $toString: '$customerId' }, userId] }`)
  - 문서 이름 변경 API: `/api/personal-files/documents/:documentId/rename` 엔드포인트 신규 추가
  - 이름 변경 시 `filename`과 `upload.originalName` 모두 업데이트 (UI에서 originalName을 표시하기 때문)

#### 4. 문서 등록 (DocumentRegistrationView) ✅
- **완료일**: 2025-12-19
- **변경**: 5초 폴링 (180초 대기) → SSE 1회성 연결
- **트리거**: n8n OCRWorker가 OCR 완료/실패 시 aims_api webhook 호출
- **파일**:
  - `server.js`:
    - SSE 엔드포인트 (`/api/documents/:documentId/status/stream`)
    - webhook 엔드포인트 (`/api/webhooks/document-processing-complete`)
  - `waitForDocumentProcessing.ts`: SSE 기반 대기 유틸리티 함수 (Promise 반환)
  - `useDocumentStatusSSE.ts`: SSE 훅 (선택적 사용)
  - `DocumentRegistrationView.tsx`: 3곳의 폴링 → SSE 전환
    - `setAnnualReportFlag`: AR 문서 처리 완료 대기
    - `linkCustomerFile`: 고객 파일 처리 완료 대기
    - `checkNormalDocumentCompletion`: 일반 문서 처리 완료 대기
  - `OCRWorker.json`:
    - `Notify Processing Complete` 노드 추가 (OCR 성공 시)
    - `Notify Processing Error` 노드 추가 (OCR 실패 시)
  - `DocPrepMain.json`:
    - `Wait 3s for SSE` 노드 추가 (SSE 연결 대기)
    - `Notify No OCR Complete` 노드 추가 (텍스트 있는 PDF)
- **특이사항**:
  - 1~3단계(지속적 목록 갱신)와 달리 **1회성 SSE** (완료 후 연결 해제)
  - 훅 대신 유틸리티 함수(`waitForDocumentProcessing`) 사용
    - 이유: 기존 코드가 함수 내부에서 동적으로 폴링 시작하는 구조
  - n8n 워크플로우 수정 필요 (JSON 수정 후 n8n UI에서 import)
  - 180초 타임아웃 유지 (서버/클라이언트 양쪽)
  - n8n 표현식에서 document_id 따옴표 포함 이슈 → server.js에서 제거 처리
- **상세 문서**: [SSE_STEP4_DOCUMENT_REGISTRATION.md](SSE_STEP4_DOCUMENT_REGISTRATION.md)

#### 5. 문서 처리 현황 (DocumentStatusProvider) ✅
- **완료일**: 2025-12-19
- **변경**: 폴링 + SSE 하이브리드 방식
- **트리거**: n8n OCRWorker가 OCR 완료/실패 시 기존 webhook에서 문서 목록 SSE 알림도 함께 발송
- **파일**:
  - `server.js`:
    - SSE 엔드포인트 (`/api/documents/status-list/stream`)
    - webhook 엔드포인트 (`/api/webhooks/document-list-change`)
    - 기존 `/api/webhooks/document-processing-complete`에서:
      - `overallStatus` 직접 업데이트 (DB에 'completed' 설정)
      - 문서 목록 SSE 알림 연동
    - `analyzeDocumentStatus()`: DB의 `overallStatus === 'completed'`면 그대로 반환
  - `useDocumentStatusListSSE.ts`: SSE 훅
  - `DocumentStatusProvider.tsx`: 폴링 + SSE 병행
- **특이사항**:
  - Provider 레벨 변경으로 앱 전역에 영향 (위험도 높음)
  - 기존 isPollingEnabled 상태는 SSE 활성화/비활성화로 의미 변경
  - Page Visibility API 처리는 SSE 훅 내부로 이동
  - Vite 프록시 설정은 기존 `^/api/.*/stream$` 패턴으로 자동 처리
- **🔴 중요: 폴링 완전 제거 불가**
  - SSE는 **완료/실패 시에만** 이벤트 발생
  - 중간 진행률(0% → 30% → 60% → 100%)은 폴링으로만 확인 가능
  - **하이브리드 방식 채택**:
    - 5초 폴링: 처리 중 진행률 업데이트
    - SSE: 완료 시 즉시 반영 (폴링 5초 대기 없이)
