# 이슈 보고서: 문서 뱃지 및 OCR 처리 문제 (v4 — 프로덕션 검증 완료)

**작성일**: 2026-03-26
**상태**: 수정 완료 + 프로덕션 검증 통과
**커밋**: `f5a757bd` (2026-03-26)
...
| 2 | **B**: 변환 파일 OCR fallback 없음 | Medium | 이미지만 포함된 PPT/HWP 등 | `doc_prep_main.py`, `extract.py` |
| 3 | **C**: xPipe 경로 OCR 필드 미기록 | Medium | xPipe 경로로 처리된 이미지 | `doc_prep_main.py` |

---

## 프로덕션 검증 결과 (2026-03-26)

### 검증 방법
Playwright e2e + MongoDB 직접 조회. 김민준 고객에게 테스트 파일 2건 업로드.

### 이슈 A + C 검증: `ocr_test_20260326.jpg` (JPG 이미지)

| 필드 | 기대값 | 실제값 | 결과 |
|------|--------|--------|------|
| 뱃지 | OCR | OCR | PASS |
| `ocr.status` | `done` | `done` | PASS |
| `ocr.full_text` | 텍스트 있음 | `"AIMSOCR Test 2026-03-26..."` | PASS |
| `docembed.status` | `done` (skipped 아님) | `done` | PASS |
| `displayName` | 자동 생성 | `AIMSOCR Test 2026.03.26.jpg` | PASS |

### 이슈 B 검증: `ocr_fallback_test.ppt` (이미지만 포함된 PPT)

| 필드 | 수정 전 (예상) | 수정 후 (실제) | 결과 |
|------|--------------|--------------|------|
| 뱃지 | BIN (보관 처리) | OCR | PASS |
| `ocr.status` | (없음) | `done` | PASS |
| `ocr.confidence` | - | `0.6797` | PASS |
| `processingSkipReason` | `conversion_failed` | (없음) | PASS |
| `upload.convPdfPath` | - | PDF 변환 성공 | PASS |
| `docembed.status` | `skipped` | `done` | PASS |

### 결론
**3건 모두 프로덕션에서 정상 동작 확인. 이슈 종결.**
