...
```
GUI 설정 (레지스트리) > 환경변수 (AC_DEV_MODE) > 자동 판별 (AC_EXE_PATH)
```

## GUI에서 개발 모드 전환 (권장)

1. `Ctrl+Shift+D` 입력
2. PIN 입력 다이얼로그에서 코드 입력
3. 타이틀바에 `[DEV]` 표시 확인

- 설정은 Windows 레지스트리에 영구 저장 (앱 재시작 후에도 유지)
- 환경변수보다 우선 적용
- 실행 중에는 토글 불가 (중지 후 전환)

## 환경변수 오버라이드 (`AC_DEV_MODE`)

어떤 실행 방식이든 환경변수로 강제 전환 가능 (GUI 설정이 없을 때):
