...
**해결된 쟁점:**
- ~~중복 업로드 방지(해시 체크)~~ → 불필요 (Alex·Gini 합의)
- ~~"나중에 처리" vs credit_pending 구분~~ → "나중에 처리" 제거로 해소

**미해결 쟁점:**
1. Phase 2 진입 시점 — 어떤 지표가 관측되면 Phase 2를 시작하는가?
2. 비용 추정 공식의 구체적 계수 — 운영 데이터 축적 후 보정 필요
3. 이탈 시 만료 정책의 구체적 시간 (Gini 지적) — Phase 2 설계 시 결정
4. 크레딧 부족 시 UX 메시지 — 현행 credit_pending UX 개선 (별도 이슈 가능)
...
> **방안 미확정. 코드 변경 불허.**
> 토의 진행 중. Alex·Gini 2차 비평 반영하여 설계 수정 중.
>
> **합의된 방향:**
> - Phase 0(B1~B4 버그 수정)이 최우선이며 즉시 실행 대상
> - Phase 2(비용 투명성 UX)는 보류 — Phase 0만으로 안전 확보 가능
> - "나중에 처리" 옵션은 "보관만"과 동일 문제이므로 제거
