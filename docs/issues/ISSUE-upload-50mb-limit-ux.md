...
> - Phase 0(B1~B4 버그 수정) 완료
>
> **미결:** Phase 2 상세 설계 (상태 다이어그램, UX 흐름, 임계값 초기값 등)

---

## Phase 1 상세 설계 (2026-02-23 토의 확정)

### 1. 핵심 정책 변경

**단일 파일 크기 제한 제거.** 사용자 저장 용량(티어별 쿼터)만으로 업로드를 제한한다.

| 항목 | 현재 | 변경 후 | 변경 사유 |
|------|------|---------|-----------|
| 단일 파일 크기 제한 | 50MB (`maxFileSize`) | **없음** | 파일 크기 ≠ 비용. 용량 쿼터가 곧 크기 제한 |
| 업로드 제한 기준 | 파일 크기 | **사용자 잔여 저장 용량** (`remaining_bytes`) | 티어별 쿼터로 충분 |
| `max_batch_upload_bytes` | 티어별 100MB~2GB | **제거** | 원래 OOM 방지 목적이었으나 스트리밍 저장으로 해소 |
| 미지원 파일 (ZIP 등) | HTTP 415 거부 | **업로드 허용, 보관만** | AIMS에 파일 보관 기능 제공 |
| Nginx `/shadow/` | `client_max_body_size 50M` | **제거** (서버 블록 `10G` 상속) | 인프라 보호는 서버 블록 10G로 충분 |
| 프론트엔드 크기 검증 | `file.size > 50MB` → 거부 | `파일 합계 > remaining_bytes` → 사용자 선택 | 용량 기반으로 전환 |
| 백엔드 크기 검증 | `MAX_UPLOAD_SIZE_MB: 50` | **용량 쿼터 기반 체크** | 크레딧과 마찬가지로 실제 자원 기반 검증 |

**"단일 파일 크기 제한이 왜 불필요한가" 근거:**
- 사용자 티어별 저장 용량 쿼터가 이미 존재함 (free_trial 512MB ~ vip 80GB)
- 쿼터 잔여량 ≥ 파일 크기이면 허용, 미만이면 거부 — 별도의 단일 파일 상한이 필요 없음
- 만약 별도 상한을 두면 "용량 남았는데 왜 안 되냐"는 불만 발생
- 서버 인프라 보호는 Nginx 서버 블록 `client_max_body_size 10G`로 충분 (사용자에게 비노출)

**`max_batch_upload_bytes`가 왜 불필요한가 근거:**
- 원래 서버 부하(OOM) 방지 목적으로 설정된 제한
- Phase 0에서 스트리밍 저장(`_stream_upload_to_disk()`)이 완료되어 OOM 위험 해소
- 제거하지 않으면 standard 사용자가 저장 용량 19GB 남아있어도 600MB 파일을 못 올리는 모순 발생

---

### 2. 파일 분류 및 처리 정책

업로드된 파일은 시스템이 처리할 수 있는지에 따라 자동 분류된다:

| 분류 | 예시 | AI 처리 | 비용 |
|------|------|---------|------|
| **텍스트 추출 가능** | PDF(텍스트), TXT, HWP, DOCX, XLSX | 텍스트 추출(무료) → 임베딩+요약(유료) | 크레딧 소비 |
| **OCR 필요** | 이미지(JPG/PNG/TIFF), 스캔 PDF | OCR(유료) → 임베딩+요약(유료) | 크레딧 소비 (OCR 추가) |
| **처리 불가** | ZIP, RAR, 바이너리, 동영상, 실행파일 등 | **없음** (보관만) | 디스크만 소비 |

**처리 불가 파일이 업로드되면:**
1. 파일은 정상적으로 디스크에 저장됨
2. DB 문서 레코드에 `processingSkipReason: "unsupported_format"` 설정
3. AI 파이프라인(OCR, 임베딩, 요약) 전부 스킵
4. `overallStatus: "completed"` (업로드 자체는 성공)
5. UI에서 "지원하지 않는 형식" 표시
6. 사용자는 파일을 다운로드/삭제할 수 있음 (AI 검색만 불가)

---

### 3. `processingSkipReason` 필드 설계

문서 레코드(`files` 컬렉션)에 **처리 스킵 사유** 필드를 추가한다.

```
processingSkipReason: null | "unsupported_format" | "user_choice" | ...
```

| 값 | 의미 | 설정 시점 | AI 처리 |
|---|------|----------|---------|
| `null` | 정상 처리 대상 | 기본값 | 진행 |
| `"unsupported_format"` | 시스템이 지원하지 않는 파일 형식 | 업로드 시 MIME 타입 판별 | 스킵 |
| `"user_choice"` | 사용자가 AI 처리를 원하지 않음 | (미래 확장) | 스킵 |

**판단 로직:**
```
if processingSkipReason === null → AI 파이프라인 진행
if processingSkipReason !== null → 보관만, UI에 사유 표시
```

**`"unsupported_format"` 선택 이유:**
- "읽을 수 없다(`unreadable`)"가 아닌 "형식을 지원하지 않는다" — 사실 진술
- 향후 ZIP 파싱 등을 지원하면 해당 파일에서 이 값을 제거하면 됨 (확장 가능)
- `"not_analyzable"`은 부적절 — 이미지 파일은 OCR로 분석 가능한데 혼동 유발

**기존 `UNSUPPORTED_MIME_TYPES` 거부 로직과의 관계:**
- 현재: MIME 타입이 `UNSUPPORTED_MIME_TYPES`에 해당 → HTTP 415 거부, 파일 저장 안 됨
- 변경: MIME 타입이 `UNSUPPORTED_MIME_TYPES`에 해당 → 파일 저장 + `processingSkipReason: "unsupported_format"` 설정
- 거부 로직이 분류 로직으로 전환됨
...
#### 프론트엔드 (UX 목적)
- 파일 선택 시 `GET /api/users/me/storage` 호출 → `remaining_bytes` 조회
- 선택된 파일 크기 합계 > `remaining_bytes` → 용량 부족 안내
- 사용자가 파일을 제외하여 용량에 맞출 수 있음
- 불필요한 대용량 전송을 사전 방지

#### 백엔드 (보안/무결성 목적)
- `doc_prep_main.py`에서 스트리밍 저장 완료 후 용량 재확인
- 프론트엔드 우회 (API 직접 호출) 방어
- 동시 업로드로 인한 레이스 컨디션 대응 (프론트엔드 체크 시점 vs 실제 저장 시점 사이에 다른 파일이 업로드될 수 있음)
- 초과 시 저장된 임시 파일 삭제 + 에러 반환

---

### 5. 용량 부족 시 일괄 업로드 동작

**전체 거부가 아닌 부분 허용.** 사용자가 직접 어떤 파일을 제외할지 선택한다.

```
사용자: 10개 파일 선택 (총 5GB)
시스템: "저장 용량이 부족합니다. 잔여: 3GB"
        → 파일 목록에서 체크 해제하여 3GB 이내로 조정 가능
사용자: 4개 파일 제외 (총 2.8GB)
        → 나머지 6개 업로드 진행
```

기존 동작(50MB 초과 파일을 시스템이 자동 제외)과 달리, **사용자에게 선택권**을 준다.

---

### 6. 재시도 버튼 정책

**원칙: 재시도해서 성공할 수 있을 때만 재시도 버튼을 보여준다.**

| 에러 유형 | 재시도 가능? | 이유 |
|----------|-------------|------|
| 네트워크 에러 | O | 일시적 장애, 재전송하면 성공 가능 |
| 서버 500 에러 | O | 일시적 장애 |
| 바이러스 감지 | X | 동일 파일은 항상 감지됨 |
| 용량 부족 | X | 용량을 비우지 않으면 결과 동일 |

프론트엔드에서 사전 검증을 제대로 하면 서버의 용량 부족 에러를 볼 일이 거의 없다.
서버 용량 부족 에러는 동시 업로드 레이스 컨디션 등 예외 상황의 안전장치일 뿐이다.

---

### 7. 크기 표시 불일치 (268.1MB vs 260.1MB) — 자연 해소

원래 이슈: 파일명 옆에 268.1MB, 서버 에러 메시지에 260.1MB로 다르게 표시.

- 원인: 스트리밍 모드에서 서버가 초과 청크까지만 읽고 413 반환 → 실제 파일 크기보다 작은 값 표시
- 해소 이유: 파일 크기 제한 제거 + 프론트엔드 사전 검증으로 서버 413 에러 경로 자체가 사라짐
- 별도 수정 불필요
...
| 항목 | 현재 | 변경 | 위치 |
|------|------|------|------|
| `client_max_body_size` | `50M` | 제거 (서버 블록 `10G` 상속) | `/etc/nginx/sites-enabled/aims:46` |
| `proxy_read_timeout` | `300` | 대용량 파일 대비 상향 | `/etc/nginx/sites-enabled/aims:47` |
| `proxy_connect_timeout` | `300` | 동일 상향 | `/etc/nginx/sites-enabled/aims:48` |
| `proxy_send_timeout` | `300` | 동일 상향 | `/etc/nginx/sites-enabled/aims:49` |

서버 블록의 `client_max_body_size 10G`는 인프라 보호 상한으로 유지한다.
사용자에게는 노출되지 않는다 (어떤 보험 문서도 10GB를 넘지 않음).
...
| # | 항목 | 영역 | 상세 |
|---|------|------|------|
| 1 | Nginx `/shadow/` `client_max_body_size` 제거 + 타임아웃 조정 | 인프라 | 서버 블록 10G 상속, 타임아웃 상향 |
| 2 | 프론트엔드 `maxFileSize` 제거 → 잔여 용량 기반 체크 | 프론트엔드 | `remaining_bytes` 조회 후 비교 |
| 3 | 프론트엔드 용량 부족 시 파일 선택/제외 UX | 프론트엔드 | 부분 허용, 사용자가 파일 제외 선택 |
| 4 | 백엔드 `MAX_UPLOAD_SIZE_MB` 제거 → 용량 쿼터 기반 체크 | 백엔드 | `storageQuotaService` 연동 |
| 5 | 백엔드 MIME 거부 → `processingSkipReason: "unsupported_format"` 전환 | 백엔드 | 거부 → 분류 로직 전환 |
| 6 | `max_batch_upload_bytes` 제거 | 백엔드+프론트 | `storageQuotaService` + 프론트엔드 |
| 7 | 용량 부족 에러 시 재시도 버튼 숨김 | 프론트엔드 | `retryable: false` 설정 |
...
## Phase 2 핵심 요구사항: 고비용 AI 처리 사전 확인 (2026-02-23 기록)

### 문제 인식

현재 크레딧 안전장치(`credit_pending`)는 크레딧이 **완전히 바닥났을 때**만 방어한다.
그러나 다음 시나리오는 방어하지 못한다:

> **크레딧이 충분히 남아 있는 상태에서, 단일 파일이 월 크레딧의 대부분을 한 번에 소진하는 경우.**

예시:
- Standard 사용자 (월 2,000 크레딧), 잔여 1,800 크레딧
- 500페이지 스캔 PDF 업로드 → OCR 1,000 + 임베딩+요약 500 = **~1,500 크레딧 소비**
- 크레딧이 충분하므로 `credit_pending`에 걸리지 않고 **자동 처리 진행**
- 결과: 파일 1개가 월정액의 **75%를 소진** — 사용자는 이 사실을 사후에야 인지

### 요구사항

**일정 기준 이상의 크레딧을 소비할 것으로 예상되는 파일은, AI 처리 시작 전에 사용자에게 확인을 받아야 한다.**

- 사용자에게 예상 비용을 보여주고, AI 처리 여부를 **선택하게** 한다
- 대부분의 일반 문서(2~30페이지)는 기존처럼 자동 처리 (사용자 개입 없음)
- 고비용 파일에서만 확인 → AIMS "던지면 알아서" 원칙 유지

### 설계 방향 (토의 확정)

| 항목 | 내용 |
|------|------|
| **임계값 기준** | 월정액의 10% (Standard 200, Pro 500 크레딧) — configurable |
| **확인 UI** | 업로드 후 또는 문서 상세에서 "이 문서는 약 N 크레딧입니다. 처리할까요?" |
| **선택지** | [처리하기] / [취소] 만. "나중에" 옵션 금지 (보관만 재발 방지) |
| **일괄 업로드** | 고비용 파일 복수 시 일괄 요약 1회 (개별 다이얼로그 금지) |
| **비용 표시** | 범위로 표시 ("예상 200~400 크레딧") — 정밀 수치 불가 |
| **크레딧 부족과의 관계** | 크레딧 부족 → 기존 `credit_pending` 유지, 고비용 확인과 별개 |
| **상태 설계** | 새 상태 추가 금지 — `completed` + `embedding = null` 또는 프론트엔드 책임 |

### 미결 사항 (Phase 2 착수 시 상세 설계)

| # | 항목 | 비고 |
|---|------|------|
| 1 | 상태 전이 다이어그램 | 고비용 확인 대기 → 처리/취소 흐름 |
| 2 | 임계값 초기값 | 페이지 수 기준 vs 예상 크레딧 기준 |
| 3 | 비용 추정 공식 | `(페이지수 × 페이지당토큰 × 단가) + (스캔이면 OCR 단가)` |
| 4 | 사용자 판단 UX 흐름 | 업로드 시점 vs 문서 상세 버튼 vs 비동기 알림 |
| 5 | 일괄 업로드 확인 UI | 일괄 요약 1회 상세 |
| 6 | 이탈 시 만료 정책 | 확인 대기 상태 TTL |
| 7 | 임계값 변경 시 소급 적용 여부 | TOCTOU 대응 |
| 8 | 시스템 측 비용 상한 | 단일 파일 = 월정액의 50% (현재 합의) |

---

## Phase 2 폐기 결정 (2026-02-24)

### 결정

**Phase 2 (고비용 AI 처리 사전 확인)를 폐기한다.**

파일 사이즈와 관계없이, `unsupported_format`이 아닌 모든 파일은 기존과 동일하게 AI 처리(OCR, 임베딩, 요약)를 자동 진행한다.

### 근거

1. **오버 엔지니어링**: AI 처리 비용이 클 것이라는 우려로 임계값 기반 사용자 확인 UX를 설계했으나, 현 시점에서 실제 고비용 파일 발생 건수가 0건이다. 존재하지 않는 문제를 해결하는 것은 과잉 설계이다.
2. **AIMS 핵심 가치 유지**: "던지면 알아서 처리"가 AIMS의 핵심 가치. 사용자에게 처리 여부를 묻는 것 자체가 이 가치를 훼손한다.
3. **기존 안전장치 충분**: Phase 0에서 수정한 크레딧 시스템(fail-closed, 정확한 페이지 수 기반 체크)과 `credit_pending` 메커니즘이 비용 통제를 담당한다. 크레딧 부족 시 자동으로 처리가 보류되므로 별도의 사전 확인이 불필요하다.
4. **단순화 원칙**: 복잡한 임계값 설정, 사용자 판단 UI, 상태 관리를 모두 제거함으로써 시스템 복잡도를 대폭 줄인다.

### 최종 처리 정책

| 파일 분류 | AI 처리 | 비고 |
|-----------|---------|------|
| 지원 형식 (PDF, 이미지, Office 등) | **자동 처리** | 기존과 동일, 사용자 개입 없음 |
| 미지원 형식 (ZIP, RAR, 바이너리 등) | **스킵** | `processingSkipReason: "unsupported_format"`, 보관만 |
| 크레딧 부족 | **보류** | `credit_pending`, 충전 시 자동 재처리 |

### 폐기된 항목

Phase 2에서 계획했던 다음 항목들은 모두 폐기:
- 고비용 임계값 (월정액의 10%)
- 사용자 판단 UI ([처리하기] / [취소])
- 비용 추정 공식 및 범위 표시
- 일괄 업로드 고비용 확인 UI
- 시스템 측 비용 상한 (단일 파일 = 월정액의 50%)
- 상태 전이 다이어그램 (고비용 확인 대기)
- 이탈 시 만료 정책

### 재검토 조건

운영 중 다음 상황이 발생하면 재검토:
- 단일 파일이 월 크레딧의 30% 이상을 소비하는 사례가 반복 발생
- 사용자로부터 "예상치 못한 크레딧 소진" 불만이 접수
...
| Phase | 상태 | 완료일 |
|-------|------|--------|
| Phase 0: B1~B4 버그 수정 + 스트리밍 업로드 | **완료** | 2026-02-23 |
| Phase 1: 50MB 제한 제거 + 저장 용량 쿼터 기반 관리 | **완료** | 2026-02-23 |
| Phase 2: 고비용 AI 처리 사전 확인 | **폐기** | 2026-02-24 |
