...
| Phase 4 | !important 제거 + 하드코딩 색상 정리 | **완료** (29/29 Playwright 통과) | `f1c285c0` |
| Phase 5 | 문서화 + 스킬 등록 | **완료** | - |
...
## Phase 4: !important 제거 + 하드코딩 색상 정리 (완료)

### 핵심 변경: !important 28건 제거 + 하드코딩 색상 ~200건 CSS 변수화

**3개 Sub-Phase로 분할 실행**, 각각 `npm run build` + Playwright 29/29 검증 완료.

### Phase 4a: !important 제거 (28건 → 0건, 9개 파일)

**전략**: @layer 시스템 + 셀렉터 specificity 조정으로 !important 없이 동일한 우선순위 확보

| 파일 | 건수 | 전략 |
|------|------|------|
| OnboardingTour.css | 15 | 단순 제거 (`.onboarding-tour__icon .sf-symbol` (0,2,0) > `.sf-symbol--size-*` (0,1,0)) |
| CorporateContractsTab.css | 8 | `.cc-product` → `.corporate-contracts-row .cc-product` 셀렉터 조정 |
| FamilyContractsTab.css | 8 | 동일 패턴 (`.fc-product` → `.family-contracts-row .fc-product`) |
| ArFileTable.css | 15 | `.ar-file-table__table tbody` 접두사 추가 (zebra stripe 대응) |
| DocumentStatusList.cells.css | 3 | 단순 제거 (source order 승리) |
| DocumentsTab.features.css | 2 | `button.document-conversion-badge--retrying` specificity 매칭 |
| DocumentStatusList.badges.css | 2 | 동일 |
| CustomerReviewModal.layout.css | 2 | 단순 제거 (source order 승리) |
| CustomerRegistrationView.css | 1 | `:focus-within` 패턴 확장 |

**검증**: ✅ `npm run build` 성공 + Playwright 29/29 통과

### Phase 4b: ChatPanel 색상 토큰화 (~161건, 7개 파일)

**Step 1: tokens.css에 ~30개 새 토큰 추가**
...
/* iOS color aliases (기존 코드에서 하드코딩으로 사용되던 값) */
...
/* Error scale extension */
--color-error-400: #f87171;
```

**Step 2: ChatPanel 6개 분할 파일에서 하드코딩 색상 → var() 치환**

| 파일 | 변경 수 | 주요 매핑 |
|------|---------|----------|
| ChatPanel.input.css | ~52 | `rgba(139,92,246,X)` → `var(--color-chat-accent-alpha-XX)` |
| ChatPanel.welcome.css | ~29 | `#8B5CF6` → `var(--color-chat-accent)` |
| ChatPanel.extras.css | ~23 | `#a78bfa` → `var(--color-chat-accent-light)` |
| ChatPanel.layout.css | ~17 | dark theme rgba → var() |
| ChatPanel.sessions.css | ~15 | `rgba(6,182,212,X)` → `var(--color-chat-cyan-alpha-XX)` |
| ChatPanel.responsive.css | ~10 | dark theme colors |

**제외**: `var(--xxx, rgba(...))` 형태의 fallback은 이미 올바른 패턴

**검증**: ✅ `npm run build` 성공 + Playwright 29/29 통과

### Phase 4c: 기타 파일 하드코딩 색상 정리 (~30건, 8개 파일)

| 파일 | 변경 수 | 매핑 |
|------|---------|------|
| DocumentStatusList.badges.css | ~20 | `#007aff` → `var(--color-icon-blue)`, `#ff9500` → `var(--color-ios-orange-light)` 등 |
| CustomerRelationshipView.css | 5 | `#0078d4` → `var(--color-accent)` (로컬 CSS 변수 참조) |
| ContractsTab.layout.css | 2 | `#f5f6f7` → `var(--color-bg-tertiary)` |
| ContractAllView.header.css | 1 | `#4a5568` → `var(--color-neutral-600)` |
| AllCustomersView.header.css | 1 | `#4a5568` → `var(--color-neutral-600)` |
| AllCustomersView.items.css | 1 | `#0055a5` → `var(--color-primary-700)` |
| ExcelRefiner.layout.css | 2 | `#f87171` → `var(--color-error-400)` |
| RegionalTreeView.css | 1 | `#0055a5` → `var(--color-primary-700)` |

**의도적 제외 (변경 안 함)**:
- `LoginPage.css` — 카카오/네이버 브랜드 색상 (가이드라인 고정값)
- `InquiryView.css` — 카카오톡 브랜드 색상
- `viewer-common.css` — 바이러스 오버레이 (테마 무관, 항상 다크 오버레이+흰 텍스트)
- `DocumentContentSearchModal.css` — 파일 유형별 Material Design 색상, 검색 하이라이트 (이미 light/dark 대응)
- `appleConfirm.css` — 전부 var() fallback 패턴 (이미 올바름)
- box-shadow의 rgba — 시각적 미세조정 값, 토큰화 부적합
...
| ChatPanel 색상 치환 | 6개 |
| 기타 색상 치환 | 8개 |
| **합계 (중복 제외)** | **23개 파일** (+551줄/-513줄) |

### 검증

| 항목 | Phase 4a | Phase 4b | Phase 4c |
|------|----------|----------|----------|
| `npm run build` | ✅ 성공 | ✅ 성공 | ✅ 성공 |
| Playwright | ✅ 29/29 | ✅ 29/29 | ✅ 29/29 |
...
### 핵심 변경: CSS_SYSTEM.md v2.0.0 전면 개정 + css-rules 스킬 업데이트

Phase 0~4에서 도입된 모든 아키텍처 변경사항을 문서화하고, Claude Code 스킬에 반영.

### 수정된 파일 (2개)

| 파일 | 변경 내용 |
|------|-----------|
| `frontend/aims-uix3/docs/CSS_SYSTEM.md` | v1.1.0 → v2.0.0 전면 개정 |
| `.claude/skills/css-rules/SKILL.md` | @layer, --grid-cols, 500줄 제한, 분할 규칙 추가 |

### CSS_SYSTEM.md v2.0.0 반영 내용

| 섹션 | 내용 |
|------|------|
| @layer 시스템 | 8개 Cascade Layer 순서, 사용 규칙, Vite 플러그인 |
| --grid-cols 패턴 | Single Source 패턴 Before/After 비교 |
| 파일 분할 규칙 | 500줄 Hard Limit, 명명규칙 8종, 자동화 도구 |
| Color Token System | Level 1~3 계층, ChatPanel/iOS 토큰 추가 |
| @layer views 패턴 | God Object 해체 → Context Via Props 패턴 |
| Playwright 테스트 | 28개 시각적 회귀 테스트 가이드 |
| Quality Checklist | 새 CSS 작성, 칼럼 추가, 코드 리뷰 체크리스트 |

### css-rules 스킬 업데이트 내용

| 추가 항목 | 설명 |
|-----------|------|
| @layer 필수 래핑 | 모든 CSS 파일은 @layer로 래핑 |
| --grid-cols 패턴 | 테이블 칼럼 Single Source |
| 500줄 Hard Limit | 초과 시 분할 필수 |
| 분할 명명 규칙 | `.layout.css`, `.table.css` 등 8종 |
| 칼럼 추가 체크리스트 | 4단계 검증 절차 |
| CSS 변수 정의 위치 | `variables.css` → `tokens.css` 수정 |
...
| 2026-02-18 20:30 | Phase 4a 완료: !important 28건 제거 (9파일), Playwright 29/29 통과 |
| 2026-02-18 21:00 | Phase 4b 완료: ChatPanel 토큰 추가 + 색상 치환 (~161건, 7파일), Playwright 29/29 통과 |
| 2026-02-18 22:00 | Phase 4c 완료: 기타 파일 색상 정리 (~30건, 8파일), Playwright 29/29 통과 |
| 2026-02-18 22:30 | Phase 4 전체 완료 (23파일, +551/-513), 커밋 대기 |
| 2026-02-18 23:00 | Phase 4 커밋 완료: `f1c285c0` (24 files, +662/-517) |
| 2026-02-18 23:30 | Phase 5 완료: CSS_SYSTEM.md v2.0.0 전면 개정 + css-rules 스킬 업데이트 |
