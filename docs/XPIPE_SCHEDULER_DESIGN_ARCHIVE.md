...
> 최종 업데이트: 2026-03-20
> 상태: **검토안** (확정 아님, 구현 미착수)
> 이 문서는 토의 과정에서 나온 검토안을 기록한 것이며, 최종 설계가 아닙니다.
...
```
Pipeline.run() → RuntimeError raise (pipeline.py:253)
  ↓ (raise 직전에 error 이벤트 발행: pipeline.py:244)
server._run_pipeline() except → doc["error"], doc["error_stage"] 기록
  ↓ (catch 후 다시 error 이벤트 발행: server.py:346) ← 중복!
EventBus → SSE → 프론트엔드 refreshDocuments()
```
...
| # | 문제 | 심각도 |
|---|------|--------|
| 1 | **에러 이벤트 2회 발행** — Pipeline 내부 + server.py catch 양쪽에서 동일 에러 발행 | Minor |
| 2 | **에러 시 파이프라인 뱃지 미표시** — "에러" 텍스트만 보이고, 어느 스테이지까지 진행했는지 안 보임 | Major |
| 3 | **skip_on_error 부분 실패가 Silent** — completed 상태로 완료되지만, 일부 스테이지 에러가 호출자에게 전달 안 됨 | Major |

### 해결 방향

**에러 레벨 분류 도입:**
- `FATAL`: Pipeline RuntimeError 중단. `Job.status = "failed"`, `Job.error` + `Job.error_stage`
- `PARTIAL`: skip_on_error=True 부분 실패. `Job.status = "completed_with_warnings"`, `Job.warnings` 필드에 스테이지별 에러 목록

**Scheduler 개입 시 주의:**
- `_run_pipeline`의 try/except를 Scheduler가 감싸서 에러를 삼키면 안 됨
- Scheduler는 Pipeline 예외를 받아 Job 상태만 관리

---

## 21. Graceful Shutdown (검토 완료)

### 현재 상태

`on_shutdown()`은 `temp_dir` 삭제만 수행. 처리 중 Task에 대한 처리 없음.
uvicorn 종료 시 미완료 Task는 `CancelledError` → doc 상태가 "processing"으로 영구 고정.
(Python 3.9+ CancelledError는 BaseException이므로 `except Exception`에 걸리지 않음)

### 설계 확정

```python
async def shutdown(wait: bool = True, timeout: float = 30) -> bool:
    """
    1. 새 작업 수신 중단
    2. 처리 중 작업 완료 대기 (최대 timeout초)
    3. 타임아웃 초과 시 강제 취소 → Job.status = "interrupted"
    4. 큐 대기 작업 로그 (유실 경고)
    5. 임시 파일 삭제
    Returns: True면 모든 작업 정상 완료, False면 타임아웃 강제 종료
    """
```
...
```
PDF 변환 (libreoffice): ~15초
OCR (Upstage API): ~10초
LLM 분류 (OpenAI): ~10초
전체 파이프라인 합산: ~25초 + 여유 5초 = 30초
```

### 큐 백엔드별 내구성

| 큐 백엔드 | shutdown 후 대기 작업 | 처리 중 작업 |
|-----------|---------------------|-------------|
| InMemoryQueue | 유실 (허용 — 데모 전용) | 강제 취소 |
| FileSystemQueue | 보존 → 재시작 후 재처리 | 재시작 후 pending 복귀 |
| MongoDBQueue | 보존 → 재시작 후 재처리 | 재시작 후 pending 복귀 |

**AIMS 프로덕션에 InMemoryQueue 배포 금지.**

---

## 22. 동시성 카운터 원자성 (검토 완료 — Gini FAIL)

### asyncio 단일 프로세스: 안전

`_running += 1`은 `await` 포인트가 없으므로 인터럽트 불가. 원자적으로 동작.

### TOCTOU 함정 (Gini 발견)

```python
# 위험한 패턴
if await self._can_start_next():  # ← await 포인트! 다른 코루틴 실행 가능
    self._running += 1             # 이미 _running이 바뀌었을 수 있음
...
**`_can_start_next()`를 async가 아닌 동기 메서드로 변경하고, 체크+증가를 단일 동기 블록으로 묶는다.**

### ProcessWorkerPool: 카운터 작동 불가

각 프로세스가 독립 메모리 → `_running` 카운터 공유 불가.
자식 프로세스 크래시 시 완료 신호 유실 → 카운터 영구 고정 → **Starvation (복구 불가)**

**해결 방향:**
- AsyncWorkerPool: 정수 카운터 + try/finally (충분)
- ProcessWorkerPool: `multiprocessing.Value` 또는 IPC 기반 완료 신호 + watchdog 타이머

### 카운터 불일치 영향

| 불일치 | 결과 |
|--------|------|
| 실제보다 높게 고정 | 새 작업 미시작 → **무한 대기 (Starvation)** |
| 실제보다 낮게 유지 | max_concurrency 초과 실행 → **API rate limit 폭발** |

### 자가 복구 메커니즘 필요

- 작업 시작 후 `max_processing_timeout`초 경과 시 강제 완료 처리
- `get_stats().processing > max_concurrency`이면 경보

---

## 23. 큐 백엔드 전환 (검토 완료 — Gini PASS with Minor)

### 전환 시나리오

| 전환 | 대기 작업 | 위험 |
|------|----------|------|
| InMemory → FileSystem | 유실 (인메모리 소멸) | xPipeWeb: 허용 |
| FileSystem → InMemory | 디스크에 방치 (아무도 안 처리) | **오판 위험** — 처리된 것으로 착각 |
| MongoDB → xPipe Scheduler | pending 작업 미이관 시 유실 | AIMS: **허용 불가** |

### 전환 절차 (확정)

```
1. 신규 작업 수신 중단
2. 현재 큐 drain 확인: queued == 0 AND processing == 0
3. 조건 미충족 → 전환 금지, drain 완료까지 대기
4. 큐 백엔드 교체
5. 신규 작업 수신 재개
```
...
class Scheduler:
    async def start(self):
        ...
        if self._queue.qsize() > 0:
            logger.warning("큐에 잔여 작업 %d건 존재", self._queue.qsize())
```

### AIMS Phase 1→2 전환 특별 조건

- 사전 공지 + 야간 점검 시간에 실행
- MongoDB pending 작업 0건 확인 후 전환
- MongoDBQueue 코드는 즉시 삭제하지 않고 롤백 가능하게 유지
