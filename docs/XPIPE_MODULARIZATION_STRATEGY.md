...
### Phase 0: 선행 조건 해결 (2-3주)

> 기간 보정: 초안 1주 → 2-3주 (Alex, Gini 리뷰 반영. E2E 테스트 작성에 최소 2주 필요)
...
| | 초안 | 보정 후 | 비고 |
|---|---|---|---|
| Phase 0 | 1주 | **2-3주** | E2E 테스트 + 보안 + God Function 분해 |
| Phase 1 | 2주 | **3주** | 어댑터 확장 + AI 호출 설계 |
| Phase 2 | 3-4주 | **5-6주** | openai_service 분리 난이도 |
| Phase 3 | 2-3주 | **2-3주** | 변동 없음 |
| Phase 4 | 1-2주 | **1-2주** | 변동 없음 |
| **합계** | **9-12주** | **13-17주** | |
...
| 리스크 | 심각도 | 제기자 | 대응 |
|--------|--------|--------|------|
| 과도한 추상화 → 개발 속도 저하 | Medium | Architect | Phase 1에서 인터페이스만 정의, 구현은 기존 코드 재사용 |
| 통합 테스트 부재 상태에서 분리 | High | Gini | Phase 0에서 파이프라인 E2E 테스트 먼저 추가 |
| **Storage 추상화 미결 → 인터페이스 재작업** | **High** | **Gini, PM** | **Phase 0에서 Q2 결정 필수. 미결 시 Phase 1 진입 불가** |
| **분류+요약 통합 AI 호출 분리 → 비용 2배** | **High** | **Alex** | **어댑터가 config만 제공, xPipe가 AI 호출 관리하는 구조로 설계** |
| **M6 분류 정확도 저하** | **High** | **PM** | **Phase 2에서 회귀 테스트로 91.8% 유지 검증** |
| **리팩토링 중 파이프라인 중단** | **High** | **PM** | **Strangler Fig 패턴으로 점진 분리. 롤백 전략 수립** |
| 보험 도메인 특화가 범용화로 희석 | Medium | PM | Layer 3에서 도메인 특화 유지, Core는 순수 인프라 |
| 인터페이스 설계 오류 (범용성 부족) | Medium | Architect | Phase 4 PoC에서 검증 후 조정 |
| **document_pipeline 인증 부재** | **Critical** | **Gini** | **Phase 0에서 인증 정책 결정. 2단계(API 제공) 전 필수 해결** |
| 기존 mock 테스트가 분리 후 무효화 | Medium | Gini | 어댑터 계약(contract) 테스트 설계로 보완 |
