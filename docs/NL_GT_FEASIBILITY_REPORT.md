...
3회 실행 결과: 68%, 69%, 68%. **프롬프트/tool description 튜닝의 한계에 도달.**

추가 개선을 위해서는 구조적 접근 필요:
1. 서버 측 상대 날짜 파라미터 (`olderThanYears`) — Q7 개선
2. 코드 레벨 후처리 (AI 응답 수치 정규화) — Q6 개선
3. 평가 스크립트 의미 기반 매칭 도입 — false negative 근본 해소
