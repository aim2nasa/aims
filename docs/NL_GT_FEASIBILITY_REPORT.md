...
```
Step 1: 방안 A 적용 (프롬프트 수정) ← 현재
Step 2: Proxy GT로 FAIL률 측정 (Q3 테스트 20건)
Step 3: 분기
  ├─ FAIL률 < 30% → UQ 종료
  └─ FAIL률 >= 30% → 방안 C 적용 → 재측정
      └─ 여전히 높으면 → 방안 B 검토
```

### Q9 대응
- Q9 전용 추가 작업 없음
- 개별 질의로 분해 시 Q3 영역은 방안 A 규칙 자동 적용
- 시스템 프롬프트에 복합 질의 분해 규칙 + Few-shot 예시 추가 (방안 D+E, A와 동시 적용)
