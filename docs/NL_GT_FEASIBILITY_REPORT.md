...
| Task | 작업 | 산출물 | 상태 |
|------|------|--------|:---:|
| 0 | GT 테스트 셋 구축 (유형별 질의+정답 쌍 30건) | `gt_test_cases.json` | 미착수 |
| 1 | MCP 도구 확장 — `list_contracts`에 집계 필드(summary), 날짜 필터/정렬 추가 | `contracts.ts` 수정 | 미착수 |
| 3 | 시스템 프롬프트에 Q4/Q6/Q7 도구 선택 가이드 + few-shot 예시 추가 | `chatService.js` 수정 | 미착수 |
| 4 | GT 기반 자동 평가 스크립트 — AI 응답 vs GT 비교 | `evaluate_sq_quality.*` | 미착수 |
| 5 | 실패 케이스 분석 → 프롬프트/도구 반복 수정 | 반복 사이클 | 미착수 |
