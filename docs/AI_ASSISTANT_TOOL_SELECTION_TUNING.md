...
### 설계 (Alex)
**프롬프트 힌트 + 도구 결과 힌트 하이브리드 방식**
- 시스템 프롬프트에 "0건이면 unified_search로 폴백" 규칙 추가 (~25줄)
- `contracts.ts`: list_contracts 0건 시 `_fallbackHint` 필드 삽입
- `customers.ts`: search_customers 0건 시 `_fallbackHint` 필드 삽입
- AI가 힌트를 보고 맥락에 맞게 폴백 여부 판단 (MAX_ITERATIONS=5 활용)
- 코드 자동 폴백 X → 과잉 폴백 방지, 기존 도구 호출 코드 미변경

### 시도 1: 프롬프트 힌트 + _fallbackHint (실패)
- 시스템 프롬프트에 폴백 규칙 25줄 추가 + contracts.ts/customers.ts에 _fallbackHint 삽입
- **결과: 85.6% (악화)** — 신규 오답 5건 + 미선택 1건 증가
- 원인: 프롬프트에 "0건이면 unified_search" 규칙 추가 → "고객명 + 문서" 질문에서 search_customers 대신 unified_search를 기대하는 방향으로 모델 행동 변화
- **즉시 롤백 완료**

### 시도 2: 코드 레벨 결과 보강 (성공!)
- chatService.js tool 실행 루프에서 list_contracts/get_contract_details 결과 0건 시 자동 unified_search
- **프롬프트 변경 0바이트**, 도구 description 변경 0바이트
- tool 결과 텍스트에 unified_search 결과를 합쳐서 AI에게 전달 (AI는 원래 도구가 풍부한 결과를 반환한 것으로 인식)
- **결과: 93.3% (84/90)** — 기준선(91.1%) 대비 **+2.2%p**, 기존 정답 미훼손
- 오답 6건 → 4건 (TC-048, TC-067 해결), 미선택 2건 유지, TC-011 신규 오답 1건

### Gini 리뷰 → Major 3건 수정
- 헬퍼 함수 선언 위치를 사용 위치 위로 이동 (TDZ 의존 제거)
- `getLastUserMessage`에 content 배열 타입 처리 + 500자 truncate
- `get_contract_details`를 FALLBACK_ELIGIBLE_TOOLS에서 제거 (dead code)
- **수정 후 결과: 92.2% (83/90)** — 기준선(91.1%) +1.1%p, TC-011 신규 오답 해소

### 최종 Phase 2 결과
| 지표 | Before (GT v4) | After (코드 폴백) |
|------|---------------|-----------------|
| 정확도 | 91.1% | **92.2%** |
| 오답 | 6건 | 5건 |
| 미선택 | 2건 | 2건 |

### RAG API 키 누락 버그 발견 및 수정
- unified_search/rag.ts의 RAG API 호출에 `x-api-key` 헤더 누락
- API 키 없이 호출 → 항상 0건 반환 (상세문서검색은 nginx가 키를 주입하므로 정상)
- `x-api-key` 헤더 추가 → unified_search 정상 동작 확인 (132건 반환)

### 실환경 테스트 결과 ✅
- "캐치업코리아 자동차 정보 알려줘" → 자동차증권, 청약서 등 문서 반환 성공!
- 이전: "자동차 관련 계약은 없습니다" (오답)
- 이후: "자동차와 관련된 문서가 다수 존재합니다" + 키워드 267건 + AI 50건
