...
M6 프롬프트로 전체 566건 재분류 + DB 반영 완료. (2026-03-10 ~ 03-11)

#### 1차 재분류 (03-10)
- 마리치(172건) + 기타(7건) + 캐치업(387건): M6 프롬프트로 DB 반영 시도
- **문제**: 여러 프로세스 동시 실행 → OpenAI 429 rate limit 유발 → 대량 `general` 오반영
- 캐치업 297/387건(77%)이 general로 잘못 분류됨 (confidence 0.0)

#### 2차 재분류 (03-10)
- 마리치(172건) + 기타(7건): 단일 프로세스로 재실행 → 정상화 완료

#### 3차 재분류 (03-11)
- 캐치업 429 에러 305건 재분류 → general 297건 → **9건** (97% 해소)
- 스크립트: `d:/tmp/fast_reclassify.py` (비동기, 순차 1건씩, 429 재시도 포함)
- 결과: 152건 처리, 143건 변경, 228초 소요

#### 최종 결과

| 고객 | 건수 | general | 상태 |
|------|------|---------|------|
| 캐치업코리아 | 387건 | **9건 (2.3%)** | 정상 — 화장품 소개, 왁싱 광고, 포트폴리오 등 실제 general |
| 마리치 | 172건 | 0건 (0%) | 정상 — 20개 유형에 골고루 분포 |
| 기타 | 7건 | 5건 | 정상 |

#### 교훈 (절대 반복 금지)
1. **OpenAI API 호출 시 프로세스 1개만 실행** — 여러 개 동시 실행하면 429 유발 → throttle 지속 → 대량 general 오반영
2. **DB 컬렉션명은 `files`** (NOT `documents`) — `docupload.files`
3. **429 에러 시 confidence 0.0** — general + confidence 0.0 = 429 에러 징후, 재분류 필요
4. **reclassify_from_db.py**: `--type general` 옵션으로 특정 유형만 재분류 가능

### 캐치업 DB 유형 분포 (387건, 3차 재분류 후 최종)

| 유형 | 건수 | | 유형 | 건수 |
|------|------|-|------|------|
| hr_document | 84 | | id_card | 12 |
| corp_basic | 36 | | diagnosis | 11 |
| medical_receipt | 35 | | general | 9 |
| insurance_etc | 33 | | personal_docs | 8 |
| corp_tax | 30 | | health_checkup | 7 |
| corp_asset | 27 | | legal_document | 5 |
| application | 26 | | coverage_analysis | 4 |
| plan_design | 24 | | 기타 | 7 |
| policy | 16 | | | |
| claim_form | 13 | | | |
