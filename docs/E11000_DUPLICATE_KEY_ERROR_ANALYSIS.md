...
## 문제 발생 시점 분석 (Git History)

### 타임라인

| 날짜 | 커밋 | 내용 | 영향 |
|------|------|------|------|
| 2025-10-25 | `72c8d029` | SHA-256 해시 저장 기능 추가 | ✅ 문제 없음 (인덱스 없음) |
| 2026-01-03 00:18 | `ac5d528c` | **시스템 전체 중복 검사 추가** | ❌ **문제 발생!** |
| 2026-01-04 | `c220531b` | 고객별 중복 허용으로 수정 | ✅ 문제 해결 |

### 문제의 시작점: `ac5d528c` (2026-01-03)

**커밋 메시지:**
```
feat: 시스템 전체 파일 해시 중복 검사 기능 추가

- 백엔드: POST /api/documents/check-hash API 추가
  - 현재 사용자(ownerId)의 모든 파일에서 동일 해시 검색
  - 고객 선택 여부와 무관하게 작동  ← ❌ 잘못된 설계!
```

**원래 구현 (문제):**
```javascript
// ❌ customerId 없이 ownerId + file_hash만 검색
const existingDoc = await db.collection(COLLECTION_NAME).findOne({
  ownerId: userId,
  'meta.file_hash': fileHash  // 고객 구분 없음!
});
```

### 설계 오류 분석

| 항목 | 잘못된 가정 | 올바른 비즈니스 규칙 |
|------|-------------|---------------------|
| 중복 범위 | 설계사당 **전역** 유니크 | **고객당** 유니크 |
| 인덱스 | `ownerId + file_hash` | `ownerId + customerId + file_hash` |
| API | customerId 파라미터 없음 | customerId 필수 |

### 교훈

1. **비즈니스 규칙 확인 필수**
   - "시스템 전체"로 일반화하기 전에 실제 요구사항 확인
   - 중복 검사 범위는 도메인 전문가와 협의

2. **커밋 메시지에 설계 의도 명시**
   - "고객 선택 여부와 무관"이라는 문구가 문제를 드러냄
   - 코드 리뷰 시 설계 의도 검토 필요
