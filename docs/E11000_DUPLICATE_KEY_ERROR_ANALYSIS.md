...
[사용자] 같은 파일 업로드 시도
         ↓
[프론트엔드] checkSystemDuplicate() 호출
         ↓
[백엔드] POST /api/documents/check-hash → isDuplicate: true
         ↓
[프론트엔드] "이미 등록된 파일입니다" 에러 표시
         ↓
[결과] 업로드 차단, E11000 방지
```
...
| 파일 | 역할 |
|------|------|
| `DocumentService.ts` | 업로드 전 중복 체크 추가 |
| `duplicateChecker.ts` | checkSystemDuplicate() 구현 |
| `server.js` | POST /api/documents/check-hash API |
