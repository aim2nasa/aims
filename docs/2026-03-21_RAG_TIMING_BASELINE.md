...
| 단계 | 소요 시간 | 전체 대비 | 비고 |
|------|-----------|-----------|------|
| 임베딩 (OpenAI API) | **0ms** | 0% | 캐시 히트 (동일 쿼리 2회째) |
| Qdrant 벡터 검색 | **144ms** | 0.4% | 12,161 포인트 대상 |
| Cross-Encoder 재순위화 | **23,123ms** | **71.4%** | 93개 결과, CPU 모드 — **최대 병목** |
| LLM 답변 생성 | **9,113ms** | 28.1% | gpt-3.5-turbo |
| **전체** | **32,381ms** | 100% | |
...
**Cross-Encoder 재순위화가 전체의 71%를 차지.** CPU 모드에서 93개 결과를 재순위화하는 데 23초 소요. 개선 방안:
