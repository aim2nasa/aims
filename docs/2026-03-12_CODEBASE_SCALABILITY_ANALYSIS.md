...
- 비용: 3~5일 | 리스크: 높음 (프로덕션 API 직접 영향)
- 선행: BUG-1~4 상태 확인 (PM 확인: 수정 완료 상태)

**필수 선행 작업: TDD 기반 안전망 구축**
1. 현재 모든 API 엔드포인트 목록 추출 (customers-routes.js에서 전수 조사)
2. **각 엔드포인트별 통합 테스트 작성** — 실제 API 호출 → 응답 구조/상태코드 검증 (pytest)
3. 테스트 ALL PASS 확인 (현재 상태 = baseline, 분할 전 동작 보장)
4. 라우트 분할 작업 (1개 그룹씩 점진적 분리)
5. **동일 테스트 ALL PASS 확인** (분할 후에도 모든 API 동일 동작 보장)
6. 배포 → 프로덕션 검증
- 테스트 없이 분할 절대 금지. 라우트 경로 누락, 미들웨어 탈락, 공유 변수 접근 실패 등을 즉시 감지해야 함
