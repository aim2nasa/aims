...
## LLM 선택: OpenAI vs Claude

### 의사결정 (2024-12-20)

| 항목 | OpenAI (GPT-4o) | Claude |
|------|-----------------|--------|
| **API 키** | 이미 있음 (임베딩용) | 추가 필요 |
| **비용 관리** | 한 곳에서 통합 | 분산 |
| **Function Calling** | 지원 (성숙) | Tool Use (성숙) |
| **Node.js SDK** | `openai` 패키지 | `@anthropic-ai/sdk` |
| **임베딩 일관성** | 동일 제공자 | 다른 제공자 |

**결정: OpenAI GPT-4o + Function Calling 사용**

이유:
- 기존 임베딩에 OpenAI API 사용 중 (API 키 이미 설정됨)
- 비용을 한 곳에서 통합 관리 가능
- 추가 API 키 관리 불필요
...
│  │  POST /api/chat                                          │   │
│  │  - 사용자 메시지 수신                                     │   │
│  │  - OpenAI API 호출 (function calling 모드)                │   │
│  │  - Function call 시 MCP 서버로 전달                       │   │
│  │  - 스트리밍 응답 반환                                     │   │
│  └─────────────────────────────────────────────────────────┘   │
└──────────────────────────┬──────────────────────────────────────┘
                           │ HTTP (function calls)
...
### 1. 일반 질문 (Function 미사용)
```
User → "안녕하세요"
  → aims_api → OpenAI API
  → GPT-4o: "안녕하세요! 무엇을 도와드릴까요?"
  → User
```

### 2. DB 조회 필요 질문 (Function 사용)
```
User → "홍길동 고객 정보 알려줘"
  → aims_api → OpenAI API (with tools)
  → GPT-4o: function_call(search_customers, {query: "홍길동"})
  → aims_api → MCP Server (HTTP)
  → MCP → MongoDB 조회
  → MCP → aims_api (결과 반환)
  → aims_api → OpenAI API (function result 전달)
  → GPT-4o: "홍길동 고객님은 개인 고객으로..."
  → User
```

### 3. 복합 질문 (다중 Function 사용)
```
User → "홍길동 고객의 만기 예정 계약 알려줘"
  → GPT-4o: function_call(search_customers, {query: "홍길동"})
  → 결과: customerId = "abc123"
  → GPT-4o: function_call(get_customer_contracts, {customerId: "abc123"})
  → 결과: 계약 목록
  → GPT-4o: "홍길동 고객님의 계약 중 3개월 내 만기 예정은..."
  → User
```
...
const openai = new OpenAI();  // OPENAI_API_KEY 환경변수 사용

// MCP 서버에서 tool 정의 가져오기 → OpenAI function 형식으로 변환
const mcpTools = await fetch('http://localhost:3011/tools').then(r => r.json());
const openaiTools = convertMcpToOpenAI(mcpTools);
...
app.post('/api/chat', async (req, res) => {
  ...
  const stream = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `당신은 AIMS(보험 설계사 관리 시스템)의 AI 어시스턴트입니다.
사용자의 질문에 친절하게 답변하세요.
  ...
  for await (const chunk of stream) {
    ...
    if (delta?.tool_calls) {
      for (const toolCall of delta.tool_calls) {
        const result = await callMcpTool(toolCall.function.name,
                                          JSON.parse(toolCall.function.arguments),
                                          userId);
        // function result를 OpenAI에게 전달하고 계속 진행
      }
    }
  }
...
// MCP tool 정의를 OpenAI function 형식으로 변환
function convertMcpToOpenAI(mcpTools) {
...
| 옵션 | 설명 | 장점 | 단점 |
|------|------|------|------|
| **A. aims_api 직접 구현** | Node.js에 OpenAI SDK 추가 | 단순, 기존 인프라 활용, API 키 공유 | - |
...
### Phase 1: 백엔드 API
1. aims_api에 `openai` 패키지 추가 (이미 설치되어 있을 수 있음)
2. `/api/chat` 엔드포인트 구현
3. MCP tool → OpenAI function 변환 로직
4. 스트리밍 응답 처리
