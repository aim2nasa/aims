...
- 코드 변경을 최소화하는 것은 중요하지만, **본질적 해결이 안 되면서 최소한만 수정하는 것은 주객전도.**
- 일이 아무리 커지더라도 본질적 방법을 먼저 검토한다. 현실적이지 않을 때만 사용자에게 상의한다.
- "최소한 수정"은 본질적 해결의 범위 안에서 적용 (불필요한 코드를 안 건드리는 것이지, 필요한 범위를 줄이는 것이 아님)
- 증상이 아닌 원인 제거. 재발 방지 설계. 미봉책/임시방편 절대 금지.
- **시간/비용을 이유로 증상만 고치는 방안을 제안하지 말 것.** 근본 해결책을 먼저 설계하고, 구현 단계를 나누는 것은 허용하되 미봉책 자체를 "해결"로 제안하는 것은 금지.
...
### Controller 위치 규칙
- `src/controllers/` — 여러 feature에서 공유하는 글로벌 controller
- `src/features/*/controllers/` — 해당 feature 전용 controller
- 컴포넌트 폴더 내 직접 배치 — 해당 컴포넌트에서만 사용하는 controller (co-location)
...
## 리팩토링 원칙 (DB Gateway 작업)

- 각 단계 완료 후 반드시 동작 확인 후 다음 단계 진행
- 기존 API 인터페이스 호환성 유지 (breaking change 금지)
- 각 Phase 작업 완료 시 CLAUDE.md 변경 이력 섹션에 기록

### 변경 이력

| 날짜 | Phase | 내용 | 커밋 |
|------|:-----:|------|------|
| 2026-04-03 | **1 완료** | Read-only DB 게이트웨이 전환 33건. aims_rag_api MongoDB 직접 연결 완전 제거. Internal API 9개 신규, regression 테스트 42건 | `bf6de606` |
| 2026-04-03 | **3 완료** | annual_report_api DB write 44건 Internal API 전환. 엔드포인트 9개(18~26번), 클라이언트 함수 7개, regression 테스트 38건. $currentDate 버그 수정 | `72d4fdcf` |
| 2026-04-03 | **4 완료** | document_pipeline DB write 52건 Internal API 전환. 엔드포인트 5개(27~31번), async 클라이언트 5개, regression 테스트 25건. maxStatus 보호 | `be22cde0` |
| 2026-04-03 | **5 완료** | Dead code 제거 + CI 아키텍처 테스트 + document_pipeline 테스트 mock 수정 (739 PASS) | `6269c362` |
| 2026-04-03 | **6 완료** | files/customers read 97건 Internal API 전환. 엔드포인트 9개(32~40번), OBJECTID_FIELDS 확장, $nin/$ne 변환, regression 테스트 36건 | `14844b9a` |
