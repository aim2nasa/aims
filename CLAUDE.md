...
### 객관적 증거 기반 개발 규칙 - 절대 준수! 📊

**코드 리뷰만으로 판단 금지! 반드시 데이터로 증명!**

1. **기능 개발/버그 수정 시:**
   - 결과가 제대로 동작하는지 **객관적 증거로 증명** 필수
   - 수정 전에 **어떤 데이터로 증명할지** 먼저 계획
   - 그 기준에 따라 동작 검증
...
**기억하라**: 데이터로 증명하지 못하면 완료가 아니다!

### 날짜/시간 표시 형식 표준 📅

**전체 시스템에서 일관된 날짜/시간 표시 형식 사용!**
...
[트랜잭션 엔티티]
  ...
  계약
    ├─ 계약 ID (PK)
    ├─ 증권번호
    ├─ FK: 설계사ID, 고객ID, 보험사ID, 상품ID
    └─ 계약이력 (1:N): 납입내역, 대출내역, 지급내역
```
...
| 엔티티 | 설명 | 식별자 |
|--------|------|--------|
| **설계사** | 보험 영업인, 여러 고객 관리 | 설계사 ID |
| **고객** | 설계사의 고객 (= 계약자), 계약 없을 수도 있음 | 고객 ID |
| **문서** | 고객이 보유한 문서 (추후 상세화 예정) | 문서 ID |
| **보험사** | 보험 상품을 제공하는 회사 | 보험사 ID |
| **보험상품** | 보험사가 판매하는 상품 (판매중/판매중지 상태) | 상품 ID |
| **계약** | 고객이 보험상품에 가입하여 증권번호를 발급받은 상태 | 계약 ID |
| **계약 이력** | 시간에 따라 누적되는 이력 (납입내역, 대출내역, 보험금지급내역 등) | 이력 ID |

> **참고**: 모든 ID는 MongoDB Document ID (`_id`)를 사용
...
| **상품 ID** | 계약 보험상품 (FK) |
| **고객 ID** | 계약자 (FK) |
| **설계사 ID** | 담당 설계사 (FK), 고객의 설계사와 동일 |
| **증권번호** | 보험사에서 발급한 계약 고유번호 |
| **계약상태** | 유효, 해지, 만기, 실효 등 |
| **피보험자** | 보험 대상자 (고객이면 고객ID, 아니면 이름 텍스트) |
| **수익자** | 보험금 수령자 (고객이면 고객ID, 아니면 이름 텍스트) |

### 관계

- **설계사 → 고객**: 1:N (한 설계사가 여러 고객 관리, 역방향은 관심 영역 밖)
- **고객 → 문서**: 1:N (한 고객이 여러 문서 보유, 추후 상세화 예정)
- **고객 → 계약**: 0:N (고객은 계약이 없거나 여러 개 보유 가능)
- **고객 ↔ 보험사**: N:M (계약을 통한 간접 관계, 한 고객이 **여러 보험사**와 계약 가능)
- **보험사 → 보험상품**: 1:N (각 보험사는 독립적으로 다양한 상품 보유)
- **계약 → 보험상품**: N:1 (계약은 특정 보험상품에 연결)
- **계약 → 이력**: 1:N (한 계약에 여러 정보 이력 기록)
...
**⚠️ 필수: 항상 로컬에서 먼저 수정 → scp로 서버에 복사**

1. **로컬에서 파일 수정 (D:\aims)**
   - 로컬 환경에서 백엔드 코드 수정
   - 사용자가 즉시 변경사항을 확인할 수 있음
   - **서버에서 직접 수정 금지!**

2. **scp로 서버에 복사**
   ```bash
   scp d:/aims/backend/api/aims_api/server.js tars.giize.com:~/aims/backend/api/aims_api/server.js
   ```
...
**중요**: 로컬 먼저 수정 → scp 복사 순서를 지켜야 사용자가 변경사항을 즉시 확인 가능!
