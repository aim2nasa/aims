...
코드 수정 후, 커밋 전에 반드시 IDE 진단(webhint) 경고를 확인하고 해결한다.
- Edit 도구 사용 후 `ide_diagnostics`가 반환되면 **즉시** 해결 (다음 작업 진행 금지)
- 흔한 위반: `aria-*` 잘못된 값, `title` 속성 누락, inline style, form label 누락
...
### 고객 삭제/휴면
기본 삭제 = 휴면 전환 (`meta.status: 'inactive'`). 완전 삭제 = `?permanent=true` (개발자 모드에서만, DB 연쇄 삭제).
