...
export const SupportMenu: React.FC = () => {
  ...
  const [step, setStep] = useState(0) // 0: 다운로드, 1: 설치 확인, 2: 완료 안내
  ...
  /** Step 2: "설치했습니다" → 설정 VBS 다운로드 + 완료 안내 */
  const handleInstallDone = useCallback(() => {
    downloadConfigVbs()
    setStep(2)
  }, [])

  /** Step 3: "확인" → localStorage 기록 + 모달 닫기 (VBS 재다운로드 없음) */
  ...
  return (
    <>
      ...
      {/* 재방문 토스트: VBS 다운로드 피드백 */}
      {toast && createPortal(
        <div className="sp-toast" onClick={() => setToast(false)}>
          다운로드 폴더의 <strong>AIMS_원격지원</strong> 파일을 더블클릭하세요
        </div>,
        document.body
      )}
        ...
        <div className="sp-overlay" onClick={handleClose}>
          <div className="sp-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="sp-wizard">
              ...
              {/* Step 3: 완료 안내 — VBS는 이미 다운로드됨, 여기서는 안내만 */}
              {step === 2 && (
                <div className="sp-card sp-card--done">
                  ...
                  <p className="sp-card-desc">
                    원격 지원이 필요할 때<br/>
                    화면 상단의 <strong>헤드셋 버튼</strong>을 누르면<br/>
                    자동으로 연결됩니다.
                  </p>
                  <p className="sp-card-note">
