...
describe('useBatchUpload - Duplicate Detection', () => {
  ...
  describe('handleDuplicateAction', () => {
    ...
    it('overwrite 액션 시 파일이 업로드됨', async () => {
      const mapping = createMockMapping()
      ...
      // overwrite 액션 실행
      act(() => {
        result.current.handleDuplicateAction('overwrite', false)
      })
    ...
    it('keep_both 액션 시 파일명이 변경되어 업로드됨', async () => {
      const mapping = createMockMapping()
      ...
      mockGetUniqueFileName.mockReturnValueOnce('test (1).pdf')
      ...
      // keep_both 액션 실행
      act(() => {
        result.current.handleDuplicateAction('keep_both', false)
      })
      ...
      expect(mockGetUniqueFileName).toHaveBeenCalled()
      expect(mockUploadFile).toHaveBeenCalled()
    })
