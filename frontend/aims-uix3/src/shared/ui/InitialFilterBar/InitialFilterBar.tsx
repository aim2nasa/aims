/**
 * InitialFilterBar
 * @description 공용 초성 필터 바 - 한글/영문/숫자 초성으로 필터링
 */

import React from 'react'
import { Tooltip } from '@/shared/ui/Tooltip'
import { SFSymbol, SFSymbolSize, SFSymbolWeight } from '@/components/SFSymbol'
import type { InitialType } from './types'
import { KOREAN_INITIALS, ALPHABET_INITIALS, NUMBER_INITIALS } from './types'
import './InitialFilterBar.css'
...
export const InitialFilterBar: React.FC<InitialFilterBarProps> = ({
  ...
  return (
    <div className={`initial-filter-bar ${compact ? 'initial-filter-bar--compact' : ''} ${className}`}>
      {/* 초성 타입 토글 */}
      ...
      <div className="initial-filter-bar__initials">
        {initials.map((initial) => {
          ...
          return (
            <Tooltip
              key={initial}
              content={hasItems ? `${initial}: ${count}${countLabel}` : `${initial}: 해당 ${targetLabel} 없음`}
              placement="bottom"
            >
              <button
                type="button"
                className={`initial-filter-bar__initial ${isActive ? 'initial-filter-bar__initial--active' : ''} ${!hasItems ? 'initial-filter-bar__initial--empty' : ''}`}
                onClick={() => onSelectedInitialChange(isActive ? null : initial)}
                disabled={!hasItems}
                aria-label={`${initial}로 시작하는 ${targetLabel}`}
