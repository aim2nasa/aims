...
const HealthCard = ({ service, status, description }: HealthCardProps) => {
...
export const SystemHealthPage = () => {
  ...
  const services = [
    {
      service: 'Node.js API',
      status: health?.nodeApi || 'unhealthy',
      description: 'AIMS 메인 백엔드 API 서버',
    },
    {
      service: 'Python API',
      status: health?.pythonApi || 'unhealthy',
      description: 'RAG 검색 및 문서 처리 API 서버',
    },
    {
      service: 'MongoDB',
      status: health?.mongodb || 'unhealthy',
      description: '데이터베이스 서버',
    },
    {
      service: 'Qdrant',
      status: health?.qdrant || 'unhealthy',
