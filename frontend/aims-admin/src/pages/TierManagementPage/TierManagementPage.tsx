...
export const TierManagementPage = () => {
  ...
  const [editCreditQuota, setEditCreditQuota] = useState<string>('');
  ...
  const updateTierMutation = useMutation({
    mutationFn: ({ tierId, quota_bytes, credit_quota }: { tierId: string; quota_bytes: number; credit_quota: number }) =>
      dashboardApi.updateTier(tierId, { quota_bytes, credit_quota }),
    onSuccess: () => {
  ...
  const handleEditStart = (tier: TierDefinition) => {
    ...
    setEditCreditQuota((tier.credit_quota ?? 2000).toString());
  };

  const handleEditSave = (tierId: string) => {
    const quotaGB = parseFloat(editQuota);
    const creditQuota = parseInt(editCreditQuota, 10);
    if (isNaN(quotaGB) || quotaGB <= 0) {
      alert('유효한 스토리지 용량을 입력하세요.');
      return;
    }
    if (isNaN(creditQuota) || creditQuota <= 0) {
      alert('유효한 크레딧 수를 입력하세요.');
      return;
    }
    updateTierMutation.mutate({ tierId, quota_bytes: Math.round(quotaGB * GB), credit_quota: creditQuota });
  };
                  ...
                  <td>
                    {editingTier === tier.id ? (
                      <div className="tier-edit-input">
                        <input
                          type="number"
                          value={editCreditQuota}
                          onChange={(e) => setEditCreditQuota(e.target.value)}
                          min="1"
                          step="100"
                          className="tier-edit-input__field tier-edit-input__field--credit"
                          aria-label="월 크레딧"
                        />
                        <span className="tier-edit-input__unit">C/월</span>
                      </div>
                    ) : (
                      <span className={tier.credit_quota === -1 ? 'tier-definition-table__unlimited' : 'tier-definition-table__quota'}>
                        {tier.formatted_credit_quota}
                      </span>
                    )}
                  </td>
                  <td>
