...
추가 기능:
- 썸네일 API: PDF/이미지의 첫 페이지를 썸네일로 반환
"""
...
# 썸네일 캐시 디렉토리 (프로젝트 내 cache 폴더)
SCRIPT_DIR = Path(__file__).parent.resolve()
THUMBNAIL_CACHE_DIR = SCRIPT_DIR / "cache" / "thumbnails"
...
async def serve_thumbnail(
    ...
    # 썸네일 생성 (캐시 사용)
    try:
        thumbnail_bytes = get_cached_thumbnail(full_path, width)
    except Exception as e:
        send_error_log("pdf_proxy", f"썸네일 생성 실패: {e}", e, {"path": str(full_path)})
    ...
    return Response(content=thumbnail_bytes, headers=headers)
