...
| 설정 | 선택지 | 설명 |
|------|--------|------|
| **어댑터** | Insurance / Legal / None | 파이프라인의 도메인 특화 플러그인. 분류 체계, 특수 문서 감지, 표시명 규칙 등을 결정 |
| **프리셋** | aims-insurance (7단계) / minimal (3단계) | 파이프라인 스테이지 조합 |
...
InsuranceAdapter (보험 도메인 플러그인)
  ├─ get_classification_config() → 보험 분류 체계 + 프롬프트
  ├─ detect_special_documents() → AR/CRS 감지
  ├─ resolve_entity() → 고객명→고객ID 매칭
  └─ generate_display_name() → "계약자_문서유형" 규칙

LegalAdapter (법률 도메인 플러그인)
  ├─ get_classification_config() → 소송/계약서/판례 분류
  ├─ detect_special_documents() → 판결문/공증서 감지
  └─ ...
```

> **현재 상태 (Phase 0):** 어댑터 ABC 인터페이스만 정의됨 (`xpipe/adapter.py`).
> InsuranceAdapter, LegalAdapter 구현체는 아직 없음.
> UI에서 선택해도 실제 동작 차이 없이 config 문자열만 저장됨.
> 구현은 Phase 2에서 기존 `doc_prep_main.py`의 보험 로직을 InsuranceAdapter로 이동할 예정.
...
4. **Classify (AI분류)** — LLM을 사용하여 문서 유형 분류 (예: 보험증권, 진단서, 청구서 등)
5. **DetectSpecial (감지)** — 특수 문서 감지 (예: AR 연간보고서, CRS 고객검토서)
...
| 항목 | 상태 | 비고 |
|------|------|------|
| DomainAdapter 구현체 | Phase 2 예정 | ABC만 정의됨. Insurance/Legal 선택해도 동작 차이 없음 |
