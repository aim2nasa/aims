...
- enabled 파라미터로 비활성화 가능 (환경변수 해석은 호스트 앱 책임)
...
class QualityGate:
    ...
    Args:
        config: 품질 임계치 설정. None이면 기본값 사용.
        enabled: 품질 게이트 활성화 여부. 기본값 True.
            환경변수(XPIPE_QUALITY_GATE 등)의 해석은 호스트 앱 책임이며,
            xPipe 코어는 os.environ을 직접 참조하지 않는다.
    """
