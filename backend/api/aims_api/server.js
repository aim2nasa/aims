...
// 공통 헬퍼 함수 (lib/helpers.js에서 추출)
const { escapeRegex, sanitizeHtml, toSafeObjectId, flattenObject, isBinaryMimeType } = require('./lib/helpers');
...
// SSE 클라이언트 관리 (lib/sseManager.js에서 추출)
 ...
 */
app.post('/api/internal/check-credit', async (req, res) => {
  // API Key 인증 (내부 호출용)
  const apiKey = req.headers['x-api-key'];
 ...
 */
app.get('/api/internal/credit-pending-documents', async (req, res) => {
  const apiKey = req.headers['x-api-key'];
  const INTERNAL_API_KEY = process.env.INTERNAL_API_KEY || 'aims-internal-token-logging-key-2024';
