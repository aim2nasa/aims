...
| **오류 누적** | 없음 (독립 계산) | 가능 (연속 이동) |
| **환경 변화 대응** | 재튜닝 필요 | 상대적으로 강함 |
| **권장 환경** | 1920x1080 고정 | 환경 변화 시 |

**결론:** 1920x1080 전체화면 고정 환경에서는 Offset 방식 권장 (기본값)
- **이유**: Offset은 각 행을 독립 계산하여 오류 누적이 없음. Arrow Down은 연속 이동 방식이라 미세한 오차가 누적될 가능성 있음.
