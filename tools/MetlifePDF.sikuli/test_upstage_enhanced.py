# -*- coding: utf-8 -*-
"""
Upstage Document Digitization API 테스트 - Enhanced 모드
더 정확한 테이블 구조 인식을 위한 Enhanced 모드 사용
"""
import os
import sys
import json
...
def call_upstage_enhanced(image_path: str) -> dict:
    ...
    print(f"[INFO] Upstage Document Digitization API 호출 중... (Enhanced 모드)")
    start = time.time()
...
def main():
    print("=" * 60)
    print("Upstage Document Digitization 테스트 - Enhanced 모드")
    print("=" * 60)
    ...
    # 디버깅용 응답 저장
    with open("upstage_enhanced_response.json", "w", encoding="utf-8") as f:
        json.dump(ocr_result, f, ensure_ascii=False, indent=2)
    print("[INFO] 원본 응답 저장: upstage_enhanced_response.json")

    # 고객 데이터 추출
    rows = extract_customer_data(ocr_result)

    print(f"\n[Upstage Enhanced 결과] {len(rows)}행 추출")
