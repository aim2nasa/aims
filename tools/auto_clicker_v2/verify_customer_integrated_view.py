...
# PROD 모드 감지 (MetlifeCustomerList.py와 동일 로직)
_is_packaged_vciv = bool(os.environ.get("AC_EXE_PATH", ""))
_VCIV_DEV_MODE = not _is_packaged_vciv
if os.environ.get("AC_DEV_MODE", "").strip() == "1":
    _VCIV_DEV_MODE = True
elif os.environ.get("AC_DEV_MODE", "").strip() == "0":
    _VCIV_DEV_MODE = False

# PROD 암호화 DiagWriter (MetlifeCustomerList.py에서 설정됨)
_VCIV_DIAG = None
...
def _focus_and_close_pdf():
    """PDF 뷰어에 포커스를 확보하고 Alt+F4로 닫기 1회 시도.

    포커스 확보 전략:
      - 기존 방식(X+80)은 AC 콘솔(x=1376~, topmost)과 겹쳐 포커스 탈취 발생
      - 수정: 아이콘 Y+150(아래쪽) = PDF 본문 영역 클릭 → AC 콘솔과 겹치지 않는 안전 영역

    Returns:
        bool: PDF 뷰어가 닫혔으면 True
    """
    # (A) 포커스 확보: PDF 본문 영역 클릭
    if exists(IMG_PDF_SAVE_BTN, 2):
        pdf_icon = find(IMG_PDF_SAVE_BTN)
        safe_x = int(pdf_icon.getCenter().getX())
        safe_y = int(pdf_icon.getCenter().getY()) + 150
        click(Location(safe_x, safe_y))
        sleep(0.5)
    else:
        # 도구 모음 자동 숨김 시: 화면 중앙 클릭 (PDF 본문 영역)
        click(Location(960, 540))
        sleep(0.5)
    ...
    if exists(yes_btn_pattern, 7):
        log(u"    예(Y) 클릭...")
        click(find(yes_btn_pattern))
        sleep(WAIT_SHORT)
    elif exists(IMG_PDF_SAVE_BTN, 0):
        # PDF 뷰어 아직 열림 → Alt+Y 키보드 폴백
        log(u"    [폴백] Alt+Y 키보드 시도")
        type("y", Key.ALT)
        sleep(WAIT_SHORT)

    # (D) 검증: PDF 저장 아이콘이 사라졌으면 닫힌 것
    sleep(WAIT_SHORT)
    return not exists(IMG_PDF_SAVE_BTN, 3)


def _taskkill_pdf():
    """PDF 뷰어 프로세스를 강제 종료하는 최후 수단.

    Returns:
        bool: 강제 종료 후 PDF가 닫혔으면 True
    """
    log(u"    [LAST RESORT] taskkill로 PDF 프로세스 강제 종료 시도")
    # ezPDF Reader 프로세스 종료 시도
    for proc_name in ["ezPDFReader.exe", "ezPDFEditor.exe"]:
        subprocess.call(["taskkill", "/F", "/IM", proc_name],
                        stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    sleep(2)
    if not exists(IMG_PDF_SAVE_BTN, 2):
        log(u"    [복구 성공] taskkill로 PDF 닫힘")
        return True
    return False
...
def navigate_save_dialog_to_dir(target_dir=None):
    ...
    """
    save_dir = target_dir or PDF_DOWNLOAD_DIR
    if not save_dir:
        return
...
def log_error(report_number, error_msg):
    ...
    if ERROR_LOG_FILE and _VCIV_DEV_MODE:
        with codecs.open(ERROR_LOG_FILE, "a", encoding="utf-8") as f:
            f.write(error_line + u"\n")
    elif _VCIV_DIAG:
        # PROD: .acdump에 에러 로그 암호화 저장
        _VCIV_DIAG.write_log(error_line)

    log(u"    [ERROR LOG] 오류 로그 저장: %s" % (ERROR_LOG_FILE or "암호화"))
...
def capture_error_screenshot(report_number, step_name):
    ...
    try:
        ...
        if _VCIV_DEV_MODE and ERROR_DIR and SCREENSHOT_DIR:
            # DEV: 평문 저장
            ...
            # PROD: .acdump에 암호화 저장
            _VCIV_DIAG.write_image(filename, capture_file)
            screenshot_path = capture_file  # temp 경로 반환
...
def copy_report_screenshots_to_error_folder(report_number):
    ...
    try:
        ...
        if not _VCIV_DEV_MODE or not SCREENSHOT_DIR or not ERROR_DIR:
            return  # PROD: 폴더 없음, 복사 불필요
...
def capture_and_exit(reason):
    ...
    try:
        ...
        if _VCIV_DEV_MODE and SCREENSHOT_DIR:
            screenshot_path = os.path.join(SCREENSHOT_DIR, filename)
            shutil.copy(capture.getFile(), screenshot_path)
        elif _VCIV_DIAG:
            _VCIV_DIAG.write_image(filename, capture.getFile())
...
def capture_step_screenshot(report_num, step_name):
    ...
    # PROD: 단계별 스크린샷 생략 (에러/크래시/OCR만 저장)
    if not _VCIV_DEV_MODE:
        return None
...
def download_annual_report():
    ...
    if ar_save_dir and os.path.isdir(ar_save_dir):
        ar_pdf_files_before = set(f for f in os.listdir(ar_save_dir) if f.lower().endswith('.pdf'))
    ...
    # ★ 포커스 확보: Y+150(아래쪽, PDF 본문) 클릭. 기존 X+80은 AC 콘솔과 겹침
    pdf_closed = False
    ...
    # 최후 수단: taskkill
    if not pdf_closed:
        pdf_closed = _taskkill_pdf()

    if not pdf_closed:
        log(u"    [FATAL] PDF 뷰어 닫기 최종 실패")
    ...
    ar_saved_files = []
    if ar_save_dir and os.path.isdir(ar_save_dir):
        ar_pdf_files_after = set(f for f in os.listdir(ar_save_dir) if f.lower().endswith('.pdf'))
...
def recover_to_report_list(report_number):
    ...
    while exists(IMG_PDF_SAVE_BTN, 2) and pdf_close_count < 15:
        ...
        _focus_and_close_pdf()
        capture_error_screenshot(report_number, "recovery_after_close_%d" % pdf_close_count)
...
def save_report_pdf(report_number):
    ...
    crs_save_dir = os.path.join(PDF_DOWNLOAD_DIR, "CRS") if PDF_DOWNLOAD_DIR else None
    ...
    try:
        ...
        if not pdf_viewer_closed:
            pdf_viewer_closed = _taskkill_pdf()
...
def verify_customer_integrated_view(pdf_save_dir=None, customer_name=None, output_dir=None):
    ...
    if output_dir and _VCIV_DEV_MODE:
        # DEV: dev/ 폴더에 평문 저장
    ...
    elif output_dir and not _VCIV_DEV_MODE:
        # PROD: 폴더 생성하지 않음, 스크린샷은 .acdump에 암호화
        SCREENSHOT_DIR = None
        ERROR_DIR = None
        ERROR_LOG_FILE = None
        LOG_FILE = None
    ...
    if pdf_save_dir:
        ...
        # AR / CRS 하위 폴더 생성
        for subdir in ("AR", "CRS"):
            sub_path = os.path.join(PDF_DOWNLOAD_DIR, subdir)
    ...
    for cleanup_round in range(1, 4):
        ...
        if exists(IMG_PDF_SAVE_BTN, 1):
            log(u"    [복구] PDF 뷰어 열림 감지 → 닫기 시도 [%d/3]" % cleanup_round)
            _focus_and_close_pdf()
