...
| 7 | Backend embedding (self-healing) | pytest | `backend/embedding/tests` |
...
### Phase 7: Backend embedding — Self-Healing 테스트 (pytest)

```bash
cd /d/aims/backend/embedding && python -m pytest tests/test_self_healing.py -v --tb=short 2>&1 | tail -30
```

- credit_pending 재확인, OCR 재시도, overallStatus 불일치 수정
...
| 7 | Backend embedding (pytest) | 12 | PASS | 0.3s |
| 8 | AutoClicker regression (pytest) | 95 | PASS | 0.4s |
| 9 | Backend aims_api migration (node) | 49 | PASS | 0.5s |
| 10 | Backend shared/schema import (node) | 2 | PASS | 0.3s |

**총 테스트: ~832+개 | 전체 PASS | 총 소요: ~57s**
```
