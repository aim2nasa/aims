...
### Phase 0: dev 검증 게이트 (필수)

**prd 배포는 dev 검증을 통과한 후에만 실행한다.**

`.dev-verified` 마커 파일 존재 여부를 확인한다:

```bash
ls -la D:/aims/.dev-verified
```

- **마커 있음** → Phase 1로 진행 (마커는 배포 완료 후 삭제)
- **마커 없음** → **prd 배포 차단**. 사용자에게 안내:
  ```
  dev 검증이 완료되지 않았습니다.
  compact-fix 또는 ACE 프로세스의 검증 단계를 먼저 완료해 주세요.
  (dev: https://localhost:5177 에서 동작 확인 필수)
  ```
- **사용자가 명시적으로 "검증 스킵", "바로 배포" 요청 시** → 경고 1회 출력 후 진행 허용
...
### Phase 5: dev 검증 마커 삭제

배포 성공 시 `.dev-verified` 마커를 삭제한다 (1회용):

```bash
rm -f D:/aims/.dev-verified
```
